...
export class UserTestData {
    ...
    private getUserRepositoryStub(): tssinon.StubbedInstance<UserRepository> {
        ...
        userRepositoryStub.doesUserExist.withArgs(Sinon.match(this.getFakeRegisterInput())).returns(Promise.resolve(false));
        userRepositoryStub.doesUserExist.withArgs(Sinon.match(this.getFakeExistingUser())).returns(Promise.resolve(true));
